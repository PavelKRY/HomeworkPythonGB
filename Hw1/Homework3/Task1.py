...
for friend in data:
    uniq_items = set(data[friend])
    not_have_items = set()
    for next_friend in data:
        if next_friend != friend:
            uniq_items -= set(data[next_friend])
            if not not_have_items:
                not_have_items |= (set(data[next_friend]))
            else:
                not_have_items.intersection_update(set(data[next_friend]))
    all_items |= set(data[friend])
    uniq_items_dict[friend] = uniq_items
    not_have_items_dict[friend] = not_have_items - set(data[friend])
